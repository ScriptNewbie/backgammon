...
- Four packages only: `move-dumper`, `training-ground`, `game-engine`, `replay-player`. Do not add another top-level package.
- Packages are not scaffolded. Do not add language toolchains unless the user asks.
- Node.js ≥ 20 for TypeScript packages. If `node` / `npm` are not on PATH, run them with Docker (`node:22-bookworm`) instead of stopping. See `AGENTS.md`.
