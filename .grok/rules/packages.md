...
- Docker only, from `move-dumper/`: `npm run up`, `npm test`, `npm run dump -- --games 1 --seed 1`, `npm run down`, `npm run install:host`. Do not run `dump:inner` / `test:inner` on the host.
