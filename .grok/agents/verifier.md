...
3. If tests exist, run them **with Docker Compose from the package directory** ([ADR 0011](docs/decisions/0011-docker-only.md), commands in [README.md](README.md)). A test file with no run is not verification. Do not use host `npm`, `node`, `python`, or `pip`.
