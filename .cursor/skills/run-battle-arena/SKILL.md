...
2. From `battle-arena/`, wrap Compose with npm — never run `battle:inner` / `test:inner` on the host. `npm run up` starts teacher + game-engine. Tests: `npm test`. Battle: `npm run battle -- --matches 1 --seed 1 --length 7` (`--no-cube` for checker-only). If `npm` is missing, use the `docker compose` lines in `battle-arena/package.json`. Requires `training-ground/checkpoints/cubeless.onnx`.
