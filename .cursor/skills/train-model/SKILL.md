...
3. Split by `gameId`, not record or batch: `int.from_bytes(sha256(gameId.encode("utf-8")).digest()[:8], "big") % 100` → train `0–89`, val `90–94`, test `95–99`. Fall back to `matchId` if `gameId` is missing. Cubeless training ignores `decision == "cube"` at sample time. New dumps ([ADR 0020](docs/decisions/0020-dumper-games-no-cube.md)) are checker-only games with `gameId` only and `position.match` null. Do not treat the dead cube as live cube labels.
