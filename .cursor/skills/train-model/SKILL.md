...
8. From `training-ground/`: tests `docker compose run --rm train python -m pytest`; train with `docker compose run --rm train python -m training_ground.train --dumps /data/dumps --epochs 20 --batch-size 1024 --checkpoint-dir checkpoints` (`gpus: all` on `train`). Optional `--export-stem checkpoints/cubeless`. IDE `.venv`: `docker compose --profile install-host run --rm install-host` (Linux venv is not a host interpreter). Install CUDA torch from the cu130 index, not PyPI. Do not run host `python` or `pip`.
