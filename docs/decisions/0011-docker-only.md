...
- Each of the five packages has its own `Dockerfile` and `docker-compose.yml`. Run Compose from that package directory. Commands: [AGENTS.md](../../AGENTS.md).
...
- Do not add a sixth top-level package for Docker. Images: `node:22-bookworm` for TypeScript packages; `python:3.12-bookworm` for `training-ground` until that package pins its own PyTorch install in-image. Shared TS lives in `ts-core` ([ADR 0016](0016-ts-core.md)).
