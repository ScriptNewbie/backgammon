...
- Each package has its own `Dockerfile` and `docker-compose.yml`. Run Compose from that package directory. Host commands: [README.md](../../README.md). Do not wrap Compose in host npm.
- Bind-mount source; keep **runtime** `node_modules` in a Compose volume so Linux binaries are used inside the container.
- Each package has Compose profile `install-host`. TypeScript: `docker compose --profile install-host run --rm install-host` writes `node_modules` onto the host for IDE typechecking. Python: the same profile writes `.venv` once `pyproject.toml` or `requirements.txt` exists (`--copies`, so bind-mounts on Windows work). Do not run dumps, tests, Vite, or training against that host tree. A Linux `.venv` is not a host interpreter.
...
`AGENTS.md`, README, domain run sections, and Cursor/Grok harness files describe Compose only — no host npm wrappers, no `docker run -v "${PWD}:/app"` fallback, and no host `tsx`. `game-engine` Compose is the run path ([ADR 0017](0017-hono.md)). GPU/CUDA for `training-ground` is [ADR 0013](0013-training-cuda.md). Pinning `foochu/bgweb-api` to a digest remains open.
