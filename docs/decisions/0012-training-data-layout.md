...
Copying batches into `training-ground/data/` would duplicate dumps and let the two trees drift. Converting to a cache before a model exists adds pipeline and invalidation work too early. Splitting by record or by batch would leak correlated positions from the same match into more than one split.
...
- **Split unit is `matchId`**, not record and not batch. Deterministic bucket:
