...
- Dumps: [docs/domain/dump-format.md](docs/domain/dump-format.md) (`manifest.json` + gzip JSONL + SGF replay). Trainers read them in place via Compose (`/data/dumps`); split by `gameId` ([ADR 0012](docs/decisions/0012-training-data-layout.md)). Do not commit `move-dumper/dumps/`, `battle-arena/replays/`, `training-ground/cache/`, checkpoints, or weights (`*.pt`, `*.onnx`, `*.pte`).
