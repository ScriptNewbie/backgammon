...
`game-engine` is not scaffolded. Do not add language toolchains, `package.json`, or an HTTP server unless the user asks.
...
- Inference is ONNX in the TS engine, not Python-per-request ([ADR 0004](docs/decisions/0004-game-engine-typescript-onnx.md)). Training also writes ExecuTorch `.pte` ([ADR 0014](docs/decisions/0014-export-onnx-and-pte.md)).
- Dumps: [docs/domain/dump-format.md](docs/domain/dump-format.md) (`manifest.json` + gzip JSONL + SGF replay). Trainers read them in place via Compose (`/data/dumps`); split by `matchId` ([ADR 0012](docs/decisions/0012-training-data-layout.md)). Do not commit `move-dumper/dumps/`, `training-ground/cache/`, checkpoints, or weights (`*.pt`, `*.onnx`, `*.pte`).
